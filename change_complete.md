...
I would like you to now. 
	1. check the work you just did. 
		...
		b. Ensure that we are not leaving our self open for something that might bite us in the butt in the future.
		c. Ensure that all unit tests needed to fully test the feature were created. (If issue is fully addressed)
		d. Ensure that all unit tests pass not just the ones you created for this feature. (If issue is fully addressed)
        e. Verify that the logic of the current phase's changes is correct in isolation (e.g., via targeted tests or manual checks), accepting that full system integration may remain broken until the entire issue is finished.    
